...
# Two generations of identifier: the current NN-HXXX-NNNNN and the older
# NN-HXXX-PAN-NNNNN, which carries an extra regional segment.
FILE_RX = re.compile(r"\b(\d{2})[-\s]?(H[A-Z]{3})[-\s]?(?:([A-Z]{2}\d)[-\s]?)?([\dOolI]{5})\b")
...
def norm_file(m):
    digits = m[-1].translate(str.maketrans("OolI", "0011"))
    middle = f"{m[2]}-" if len(m) > 3 and m[2] else ""
    return f"{m[0]}-{m[1]}-{middle}{digits}"
...
def segment(pages):
    ...
    for sg in segs:
        ...
        sg["file_no_repaired"] = bool(first_hdr) and any(
            ch in m[-1] for m in FILE_RX.findall(blob[0][:400]) for ch in "OolI") if blob else False
...
if __name__ == "__main__":
    ...
    json.dump(segs, open(sys.argv[2], "w", encoding="utf-8"), indent=1,
              ensure_ascii=False)
